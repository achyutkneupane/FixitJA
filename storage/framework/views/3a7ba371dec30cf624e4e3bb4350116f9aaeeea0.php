<div id="layoutDefault_footer">
    <footer class="footer pt-website-10 pb-website-5 mt-auto bg-light footer-light">
        <div class="container-website">
            <div class="row">
                <div class="col-lg-3">
                    <div class="footer-brand">FixitJA</div>
                    <div class="mb-3">Project made easy</div>
                    <div class="icon-list-social mb-5">
                        <a class="icon-list-social-link" href="#!">
                            <i class="fab fa-instagram">
                            </i>
                        </a>
                        <a class="icon-list-social-link" href="#!">
                            <i class="fab fa-facebook"></i>
                        </a>
                        <a class="icon-list-social-link" href="#!">
                            <i class="fab fa-twitter">
                            </i>
                        </a>
                    </div>
                </div>
                <div class="col-lg-9">
                    <div class="row">
                        <div class="col-lg-3 col-md-6 mb-5 mb-lg-0">
                            <div class="text-uppercase-expanded text-xs mb-4">About FixitJA</div>
                            <ul class="list-unstyled mb-0">
                                <li class="mb-2"><a href="/about">About the comapany</a></li>
                                <li class="mb-2"><a href="/underconstruction">Fixician Directory</a></li>
                                <li class="mb-2"><a href="/underconstruction">Services</a></li>
                                <li class="mb-2"><a href="/underconstruction">How it works</a></li>
                                <li class="mb-2"><a href="/underconstruction">Hiring Process</a></li>
                            </ul>
                        </div>
                        <div class="col-lg-3 col-md-6 mb-5 mb-lg-0">
                            <div class="text-uppercase-expanded text-xs mb-4">Technical</div>
                            <ul class="list-unstyled mb-0">
                                <li class="mb-2"><a href="/underconstruction">Documentation</a></li>
                                <li class="mb-2"><a href="/underconstruction">Changelog</a></li>
                                <li class="mb-2"><a href="/underconstruction">Versions</a></li>
                            </ul>
                        </div>
                        <div class="col-lg-3 col-md-6 mb-5 mb-md-0">
                            <div class="text-uppercase-expanded text-xs mb-4">Support</div>
                            <ul class="list-unstyled mb-0">
                                <li class="mb-2"><a href="/contact">Contact</a></li>
                                <li class="mb-2"><a href="/underconstruction">FAQs</a></li>
                                <li class="mb-2"><a href="/underconstruction">Careers</a></li>
                            </ul>
                        </div>
                        <div class="col-lg-3 col-md-6">
                            <div class="text-uppercase-expanded text-xs mb-4">Legal</div>
                            <ul class="list-unstyled mb-0">
                                <li class="mb-2"><a href="/underconstruction">Privacy Policy</a></li>
                                <li class="mb-2"><a href="/underconstruction">Terms and Conditions</a></li>
                                <li><a href="/underconstruction">License</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <hr class="my-website-5" />
            <div class="row align-items-center">
                <div class="col-md-6 small">Copyright &#xA9; FixitJA 2021 Powered By <a href="https://cumbv.com" style="color: blue !important; text-decoration:underline">Cumberland Business Ventures</a></div>
                <div class="col-md-6 text-md-right small">
                    version 1.0.0
                </div>
            </div>
        </div>
    </footer>
</div>
<?php /**PATH C:\Apache24\htdocs\FixitJA\FixitJA-CBV-New\FixitJA\resources\views/layouts/partials/_footer.blade.php ENDPATH**/ ?>